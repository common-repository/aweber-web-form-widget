...
class AWeberCollection extends AWeberResponse implements \ArrayAccess, \Iterator, \Countable {
    ...
    protected function _updatePageSize() {
        # Total records in the entries will be paegSize,
        # as we are iterating over all the links
    ...
    public function iterateAllLinks() {
        if (array_key_exists('next_collection_link', $this->data)) {
            ...
            while (isset($url)) {
                ...
                for ($i=0; $i < count($info['entries']); $i++) {
                  $this->data['entries'][] = $info['entries'][$i];
                }
                $url = isset($info['next_collection_link']) ? $info['next_collection_link'] : null;
            }
            $this->_updatePageSize();
        }
